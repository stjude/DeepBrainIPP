...
def get_affine(in_file,testfile):
    return read_image(in_file).affine


def read_image_files(image_files, image_shape=None, crop=None, label_indices=None,isstructured=True):
    """
    ...
    for index, image_file in enumerate(image_files):
        ...
        image_list.append(read_image(image_file, image_shape=image_shape, crop=crop, interpolation=interpolation,isstructured=isstructured))

    return image_list


def read_image(in_file, image_shape=None, interpolation='linear', crop=None, isstructured=True):
    #isstructured=False
    #print("Reading: {0}".format(in_file), "structured: ",isstructured)
    image = nib.load(os.path.abspath(in_file))
    image = fix_shape(image)
    
    if isstructured==True:
        return image
    else:#edited by shahinur
        if crop:
