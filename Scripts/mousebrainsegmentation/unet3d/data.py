...
def create_data_file(out_file, n_channels, n_samples, image_shape):
    ...
    #filters = tables.Filters(complevel=0)#change by shahinur
...
def write_image_data_to_file(image_files, data_storage, truth_storage, image_shape, n_channels, affine_storage,
                             truth_dtype=np.uint8, crop=True, label_indices=None, save_truth=True, isstructured=True):
    for set_of_files in image_files:
        if label_indices is None:
            _label_indices = len(set_of_files) - 1
        else:
            _label_indices = label_indices
        images = reslice_image_set(set_of_files, image_shape, label_indices=_label_indices, crop=crop,isstructured=isstructured)
...
def write_data_to_file(training_data_files, out_file, image_shape, truth_dtype=np.uint8, subject_ids=None,
                       normalize=True, crop=True, save_truth=True,isstructured=True):
    """
    ...
    write_image_data_to_file(training_data_files, data_storage, truth_storage, image_shape,
                             ...
                             label_indices=label_indices, save_truth=save_truth,isstructured=isstructured)
    if subject_ids:
...
def open_data_file(filename, readwrite="r"):
    return tables.open_file(filename, readwrite,driver="H5FD_CORE",MAX_BLOSC_THREADS= 256)
