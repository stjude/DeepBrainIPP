...
def get_training_and_validation_generators(data_file, batch_size, n_labels, training_keys_file, validation_keys_file,
                                           ...
                                           validation_batch_size=None, skip_blank=True, permute=False, patch_overlap=0):
    """
...
def data_generator(data_file, index_list, batch_size=1, n_labels=1, labels=None, augment=False, augment_flip=True,
    ...
    while True:
        ...
        if shuffle_index_list:
            shuffle(index_list)
        #print("total patches: ",len(index_list),"batch size: ",batch_size)
        
        last_drop=len(index_list)%batch_size
        while (len(index_list)-last_drop)> 0:
            
            index = index_list.pop()
            add_data(x_list, y_list, data_file, index, augment=augment, augment_flip=augment_flip,
                     augment_distortion_factor=augment_distortion_factor, patch_shape=patch_shape,
                     skip_blank=skip_blank, permute=permute)
            if x_list is not None and len(x_list) == batch_size: #or (len(index_list) == 0 and len(x_list) > 0):
                yield convert_data(x_list, y_list, n_labels=n_labels, labels=labels)
                #print(np.max(np.reshape(np.array(x_list),(4,1*320*256*80)),axis=1))
                #print(np.max(np.reshape(np.array(y_list),(4,1*320*256*80)),axis=1))
                x_list = list()
                y_list = list()
